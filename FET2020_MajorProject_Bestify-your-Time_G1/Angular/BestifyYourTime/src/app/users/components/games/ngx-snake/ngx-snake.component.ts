...
import { GAME_MODES, CONTROLS, COLORS, BOARD_SIZE } from './ngx-snake.contsants';
import { BestScoreManager } from '../../../../services/ngx-snake-service.service';
...
@Component({
  selector: 'app-ngx-snake',
  templateUrl: './ngx-snake.component.html',
  styleUrls: ['./ngx-snake.component.scss'],
  host: {
    '(document:keydown)': 'handleKeyboardEvents($event)'
  }
})
export class NgxSnakeComponent {
  ...
  gameOver(): void {
    ...
    let snakeScore = new Score( 
      this.tokenService.getUser().user_id,
      1,
      this.score,
      formatDate(new Date(), 'yyyy-MM-dd','en_US') 
    )
    this.scoreService.postData(snakeScore).subscribe((reponse)=>{
      console.log("Snake Response : ",reponse);
      // this.route.navigate(['admin']);
     });

     
      //  calling fuction to send mail
      this.scoreService.sendEmail("http://localhost:8080/api/scores/sendmail",this.data,snakeScore).subscribe(
        data => {
          let res:any = data;
    ...
    if (this.score > this.best_score) {
      ...
      // let variable= JSON.parse(localStorage.getItem('ngx_snake'))
    
    // console.log("variable : " , variable);
    // console.log("best score : " , variable.best_score);
